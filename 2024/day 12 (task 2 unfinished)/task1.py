...
def find_letter_borders(letter_coordinates):
    borders = []
    for letter, coords in letter_coordinates:
        walls = 0
        for coord in coords:
            x, y = coord
            for neighbour in [(x - 1, y), (x + 1, y), (x, y - 1), (x, y + 1)]:
                if neighbour not in coords:
                    walls += 1
        borders.append((letter, (walls, len(coords)), (walls * len(coords))))
    return borders
...
print(*coordinates, sep="\n", end="\n------ compacted regions\n")
borders = find_letter_borders(coordinates)
print(*borders, sep="\n", end="\n------ borders\n")

print("Total price:", sum([border[2] for border in borders]))